tableextension 14304311 "AQD COMWarehouseActivityLine" extends "Warehouse Activity Line"
{
    fields
    {
        modify("Lot No.")
    ...
    begin
        ...
        if WhseActivityLine."Lot No." <> '' then begin
            if "Source Document" = "Source Document"::"Outbound Transfer" then begin
                ProdOrder.SetRange("AQD In-Transfer Order No.", "Source No.");
                if Location.Get(WhseActivityLine."Location Code") then if ProdOrder.FindFirst() then if Location.Get(ProdOrder."Location Code") then if Location."AQD Co-Man Location" then exit;
            end;
