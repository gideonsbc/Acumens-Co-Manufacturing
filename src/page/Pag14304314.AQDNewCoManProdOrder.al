page 14304314 "AQD New Co-Man Prod. Order"
{
        ...
        area(Content)
        {
            group(GroupName)
            {
                ...
                field("Add to Transfer Order No."; TranHCode)
                {
                    ...
                    ToolTip = 'Specifies the value of the TranHCode field.';
                    Caption = 'TranHCode';
                    ...
                    begin
                        ProductionOrder.SETFILTER("AQD Co-Man No.", Rec."No.");
                        ProductionOrder.SetFilter("AQD In-Transfer Order No.", '<>%1', '');
                        IF PAGE.RUNMODAL(0, ProductionOrder) = ACTION::LookupOK THEN BEGIN
                            if TransferHeader.Get(ProductionOrder."AQD In-Transfer Order No.") then begin
                                Text := ProductionOrder."AQD In-Transfer Order No.";
                ...
                field("Output Item"; OutItem)
                {
                    ...
                    begin
                        ...
                        if SKU.Get(CLocationCode, OutItem, '') then
        ...
        if CloseAction = CloseAction::LookupOK then begin
            if Confirm(ConLbl, true) then begin
                ...
                if InBoundTransfer then
                    if TranHCode = '' then
                        OrderMgt.InsertTransHeader(TranHeader, FLocationCode, CLocationCode, DueDate)
                    else
