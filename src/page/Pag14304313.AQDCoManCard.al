page 14304313 "AQD Co-Man Card"
{
    ...
    layout
    {
        area(Content)
        {
            ...
            part("Released Prod. Order Subform"; "AQD Released ProdOrder Subform")
            {
                Caption = 'Released Production Order Lines';
                ApplicationArea = All;
                SubPageLink = "AQD Co-Man No." = FIELD("No."), Status = const(Released);
                SubPageView = SORTING("No.");
            }
            part("Finished Prod. Order Subform"; "AQD Finished ProdOrder Subform")
            {
                Caption = 'Finished Production Order Lines';
                ApplicationArea = All;
                SubPageLink = "AQD Co-Man No." = FIELD("No."), Status = const(Finished);
                SubPageView = SORTING("No.");
            }
        }
        area(FactBoxes)
    ...
    actions
    {
        area(Processing)
        {
            action("New Co-Man Production Order")
            {
                ...
                ToolTip = 'Executes the New Co-Man Production Order action.';

                trigger OnAction()
                var
                    CoManHeader: Record "AQD Co-Man Header";
                    NewCoManPro: Page "AQD New Co-Man Prod. Order";
                begin
            ...
            action("Lot Restriction")
            {
                ...
                ToolTip = 'Executes the Lot Restriction action.';
            }
            action(CloseArchive)
            {
                ApplicationArea = All;
            ...
            action("Consumption Journal")
            {
                ...
                ToolTip = 'Executes the Consumption Journal action.';
            }
            group(Outbound)
            {
                Caption = 'Outbound';
