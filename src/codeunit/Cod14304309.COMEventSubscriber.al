codeunit 14304309 "COM Event Subscriber"
{
    ...
    begin
        if (WhseJnlLine."Source Type" = Database::"Transfer Line") and (WhseJnlLine."Source Subtype" = 0) then begin
            ProdOrder.SetRange("In-Transfer Order No.", WhseJnlLine."Source No.");
            if Location.Get(WhseJnlLine."Location Code") then
                if Location."QA. Zone" = WhseJnlLine."From Zone Code" then
                    if ProdOrder.FindFirst() then begin
                        if Location.Get(ProdOrder."Location Code") then if Location."Co-Man Location" then QASingleInstance.SetQARestriction(true);
                        ...
                        if ItemRest.FindFirst() then begin
                            if CoManHeader.Get(ProdOrder."Co-Man No.") then begin
                                if not LotRestriction.Get(ProdOrder."Co-Man No.", WhseJnlLine."Item No.", WhseJnlLine."Variant Code", WhseJnlLine."Lot No.") then begin
                                    LotRestriction.Init();
                                    LotRestriction."Co-Man No." := ProdOrder."Co-Man No.";
    ...
    begin
        if (WarehouseJournalLine."Source Type" = Database::"Transfer Line") and (WarehouseJournalLine."Source Subtype" = 0) then begin
            ProdOrder.SetRange("In-Transfer Order No.", WarehouseJournalLine."Source No.");
    ...
    var
        ...
        ProdOrder: Record "Production Order";
        Location: Record Location;
        SingleInstance: Codeunit "COM Single Instance";
    begin
                ...
                if WarehouseActivityLine."Activity Type" = WarehouseActivityLine."Activity Type"::Pick then
                    if WarehouseActivityLine."Action Type" = WarehouseActivityLine."Action Type"::Take then
                        if (WarehouseActivityLine."Source Type" = Database::"Transfer Line") and (WarehouseActivityLine."Source Subtype" = 0) then begin
                            ProdOrder.SetRange("In-Transfer Order No.", WarehouseActivityLine."Source No.");
    ...
    begin
        ...
        if not lbRecordRefHandled then begin
            case precDocumentAttachment."Table ID" of
                Database::"Co-Man Header":
                    begin
                        prrRecordRef.Open(Database::"Co-Man Header");
    ...
    var
        Location: Record Location;
    begin
        if ItemJnlLine2."Entry Type" = ItemJnlLine2."Entry Type"::Consumption then if Location.Get(ItemJnlLine2."Location Code") then if Location."Consume Blocked Lot" then IsHandled := true;
    end;
    ...
    begin
        TransRcptHeader."Co-Man No." := TransHeader."Co-Man No.";
            ...
            if Location.Get(PurchRcptHeader."Location Code") then
                if Location."PO. to TR. Order" then begin
                    ...
                    if ItemLedgerEntry.FindFirst() then begin
                        ...
                        TransferTo.SetLData(PurchRcpHdrNo, ItemLedgerEntry."Item No.", ItemLedgerEntry."Location Code", ItemLedgerEntry."Posting Date", ItemLedgerEntry.Quantity, ItemLedgerEntry."Base Unit of Measure");
    ...
    begin
        ...
        case DocumentAttachment."Table ID" of
            Database::"Co-Man Header":
                begin
                    RecRef.Open(Database::"Co-Man Header");
